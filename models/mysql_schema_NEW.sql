...
--  Table with all the players in the ACB website
CREATE TABLE player (
    id INTEGER  AUTO_INCREMENT PRIMARY KEY,  -- Table autogenerated id
    player_acbid VARCHAR(255) NOT NULL,  -- ACB player id
...
CREATE INDEX player_acbd_idx ON player(player_acbid);

-- Table with the performances of a player in an specific game
CREATE TABLE player_performance (
    ...
    -- This should be deleted with the id of the player only or an specific one for team performance???????????????
...
CREATE INDEX player_performance_game_id_idx ON player_performance(game_id);
CREATE INDEX player_performance_team_id_idx ON player_performance(team_id);
CREATE INDEX player_performance_player_id_idx ON player_performance(player_id);

-- Table with all the coaches (note some may be players and coaches)
CREATE TABLE coach (
    id INTEGER  AUTO_INCREMENT PRIMARY KEY,  -- Table autogenerated id
    coach_acbid VARCHAR(255) NOT NULL,  -- ACB coach ID.
...
CREATE INDEX coach_acbd_idx ON coach(coach_acbid);

-- Table linking the coaches with the games they were part of
...
CREATE INDEX coach_performance_game_id_idx ON coach_performance(game_id);
CREATE INDEX coach_performance_team_id_idx ON coach_performance(team_id);
CREATE INDEX coach_performance_coach_id_idx ON coach_performance(coach_id);

-- Table with all the events per game
CREATE TABLE event (
    id INTEGER AUTO_INCREMENT PRIMARY KEY,  -- Table autogenerated id
    ...
    team_code VARCHAR(255) DEFAULT null,  -- Code (short name) appearing on the site
    team_id INTEGER DEFAULT null,  -- Reference to the team id
