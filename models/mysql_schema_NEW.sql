...
CREATE TABLE event (
    id INTEGER AUTO_INCREMENT PRIMARY KEY,  -- Table autogenerated id
    event_game_id INTEGER NOT NULL,  -- Id specific for a game
    game_acbid INTEGER NOT NULL,  -- Reference to game id
    team_acbid VARCHAR(3) DEFAULT null,  -- Should be replaced with the team id, not acb id????????????????
...
CREATE INDEX event_event_game_id_idx ON event(event_game_id);